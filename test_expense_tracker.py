...
def test_load_expenses_file_not_exists():
    with patch("os.path.exists", return_value=False):
        assert expense_tracker.load_expenses() == []
...
def test_load_expenses_file_exists_with_data():
    ...
    with patch("os.path.exists", return_value=True):
        with patch("builtins.open", mock_open(read_data=json.dumps(mock_data))):
            assert expense_tracker.load_expenses() == mock_data
...
def test_add_expense_valid(capsys):
    ...
    assert len(expense_tracker.data) == 1
    assert expense_tracker.data[0] == {"amount": 100, "category": "Food", "date": "2023-01-01"}
...
def test_save_expense():
    expense_tracker.data = [{"amount": 100, "category": "Food", "date": "2023-01-01"}]
    m_open = mock_open()
    ...
    assert json.loads(written_content) == expense_tracker.data
...
def test_reset_expenses_yes(capsys):
    ...
    with patch("builtins.input", return_value="yes"):
        with patch("expense_tracker.save_expense") as mock_save:
            expense_tracker.reset_expenses()
            assert expense_tracker.data == []
...
def test_main_quit():
    with patch("expense_tracker.load_expenses", return_value=[]):
